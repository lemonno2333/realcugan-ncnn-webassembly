...
- `threadCount`: accepted range `1..8`.
- `tileSize`: accepted values `128`, `160`, or `200`.

The threaded build prewarms at most eight pthread workers. The UI then selects
...
- Desktop devices use caps of four, six, or eight threads depending on reported
  device memory.
...
```shell
node tests/benchmark-runtime-settings.cjs --backend simd-threads --threads 4,6,8
```
...
```shell
node tests/benchmark-runtime-settings.cjs --backend simd-threads --threads 2,4,6,8 --tiles 128,160,200 --cancel
```
